...
for x in train dev test; do
  ...
  for nn in `find  $corpus_dir/$x -name "*.wav" | sort -u | xargs -I {} basename {} .wav`; do
      # 按照'_' 分割，输出第一个位置的字符串, 输出 A19
      spkid=`echo $nn | awk -F"_" '{print "" $1}'`
      # 按照(A-Z)匹配，替换为该匹配项第一个字符  输出 A
      ...
      # sed -n 1p $corpus_dir/data/$nn.wav.trn找出第1行
      # sed -n 1p $corpus_dir/data/$nn.wav.trn找出第3行
      echo $uttid `sed -n 1p $corpus_dir/data/$nn.wav.trn` >> word.txt
      echo $uttid `sed -n 3p $corus_dir/data/$nn.wav.trn` >> phone.txt
  done 
  cp word.txt text
