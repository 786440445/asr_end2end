...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # local/aishell_data_prep.sh $dbase/data_aishell data/aishell || exit 1;
    local/thchs-30_data_prep.sh $dbase/data_thchs30 data/thchs || exit 1;
    ...
    if $has_aishell2; then
        ...
        tools/combine_data.sh data/train data/thchs/train || exit 1;
            # data/{aidatatang,aishell,magicdata,primewords,stcmds,thchs}/train || exit 1;data/aishell/train || exit 1;
        tools/combine_data.sh data/dev data/thchs/dev || exit 1;
    fi
    tools/perturb_data_dir_speed.sh 0.9 data/train data/train_sp0.9
    tools/perturb_data_dir_speed.sh 1.1 data/train data/train_sp1.1
    tools/combine_data.sh data/train_sp data/train data/train_sp0.9 data/train_sp1.1
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    # save average model, 保存最优的30个模型平均参数。
